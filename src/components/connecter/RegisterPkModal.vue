            ...
            <div class="m-auto w-[350px] rounded-lg p-[20px_30px] shadow-[0_2px_8px_rgba(0,0,0,0.33)] transition-all duration-300">
                ...
                <div class="modal-footer flex justify-around">
                    <slot name="footer">
                        ...
                        <button type="submit" v-if="isApply" @click="$emit('close'), $emit('registerPk', 'apply')" :class="btnBase" class="bg-blue-400">はい</button>
                        <button type="submit" v-if="isCancel" @click="$emit('close'), $emit('registerPk', 'cancel')" :class="btnBase" class="bg-blue-400">はい</button>
                    </slot>
                </div>
            </div>
        </div>
